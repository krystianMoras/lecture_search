        ...
        for sentence_batch in self.source_dp:
            ...
            for i, sentence in sentence_batch:
                ...
                if not self.is_train:
                    possible_spans = UCPhraseDataPipe.get_possible_spans(
                        word_idxs,
                        len(input_id),
                        self.MAX_WORD_GRAM,
                        self.MAX_SUBWORD_GRAM,
                    )
                len_diff = max_len - len(input_id)
                assert len_diff >= 0, "Input ids must have been sorted!"
                input_ids_batch.append(
                    [self.bos_token_id] + input_id + [self.pad_token_id] * len_diff
                )
            ...
            if self.is_train:
                yield original_sentence_ids, torch.tensor(
                    input_ids_batch
                ), torch.tensor(input_masks_batch), pos_spans_batch, neg_spans_batch
            else:
                yield original_sentence_ids, torch.tensor(
                    input_ids_batch
                ), torch.tensor(input_masks_batch), possible_spans_batch
