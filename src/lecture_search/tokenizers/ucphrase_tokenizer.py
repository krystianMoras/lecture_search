...
class UCPhraseTokenizer:
    ...
    def tokenize_text(self, text):
        ...
        tokenized_sents = [
            self.LM_TOKENIZER.tokenize(" " + s, add_special_tokens=False) for s in sents
        ]
    ...
    def tokenize_docs(
        self, docs: dict, tokenized_docs_path: Path, tokenized_id_docs_path: Path
    ):
        tokenized_docs = []
        tokenized_id_docs = []
...
if __name__ == "__main__":
    ...
    UCPhraseTokenizer().tokenize_docs(
        transcriptions,
        Path("data") / "tokenized_arxiv.json",
        Path("data") / "tokenized_id_arxiv.json",
    )
