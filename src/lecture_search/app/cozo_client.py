...
def full_text_search(cozo_client, search_string: str):
    ...
    search_string_list = [stemmer.stem(word) for word in search_string]
    ...
    for row in sentences.itertuples():
        if row.type == "video":
            ...
            sentences.loc[
                row.Index, "start_time"
            ] = sentence_result.start_time.tolist()[0]
        ...
        if row.type == "kadzinski_notes_pdf" or row.type == "pdf":
            ...
            sentences.loc[
                row.Index, "sentence_i"
            ] = sentence_result.sentence_i.tolist()[0]
...
def decode_passage(cozo_client, passage_id, passage_type):
    ...
    total_passage = " ".join(
        [
            sentences[sentences.sentence_id == sentence_id].sentence.values[0]
